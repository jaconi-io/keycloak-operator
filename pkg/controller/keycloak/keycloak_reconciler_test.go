...
func TestKeycloakReconciler_Test_Creating_All(t *testing.T) {
	...
	//    1) Database secret
	//    2) Postgresql Persistent Volume Claim
	//    3) Postgresql Deployment
	//    4) Postgresql Service
	//    5) Keycloak Service
	//    6) Keycloak Discovery Service
	//    7) Keycloak Probe ConfigMap
	//    8) Keycloak StatefulSets
	//    9) Keycloak Ingress
	...
	assert.IsType(t, model.DatabaseSecret(cr), desiredState[1].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlPersistentVolumeClaim(cr), desiredState[2].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlDeployment(cr), desiredState[3].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlService(cr, model.DatabaseSecret(cr), false), desiredState[4].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakService(cr), desiredState[5].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDiscoveryService(cr), desiredState[6].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakProbes(cr), desiredState[7].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr), nil), desiredState[8].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakIngress(cr), desiredState[9].(common.GenericCreateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Updating_All(t *testing.T) {
	...
	//    5) Keycloak Service
	//    6) Keycloak Discovery Service
	//    7) Keycloak StatefulSets
	//    8) Keycloak Ingress
	assert.Equal(t, 9, len(desiredState))
	...
	assert.IsType(t, model.DatabaseSecret(cr), desiredState[1].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlPersistentVolumeClaim(cr), desiredState[2].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlDeployment(cr), desiredState[3].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlService(cr, model.DatabaseSecret(cr), false), desiredState[4].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakService(cr), desiredState[5].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakDiscoveryService(cr), desiredState[6].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr), nil), desiredState[7].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakIngress(cr), desiredState[8].(common.GenericUpdateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Updating_External_Database_WithIPAddress(t *testing.T) {
	...
	currentState.DatabaseSecret = model.DatabaseSecret(cr)
	// This conversion is done by K8s. In the tests, we need to fake it.
	...
	for _, v := range desiredState {
		...
		} else if reflect.TypeOf(v) == reflect.TypeOf(common.GenericCreateAction{}) &&
			reflect.TypeOf(v.(common.GenericCreateAction).Ref) == reflect.TypeOf(model.PostgresqlService(cr, currentState.DatabaseSecret, true)) &&
			v.(common.GenericCreateAction).Ref.(*v1.Service).Name == model.PostgresqlServiceName {
			service = v.(common.GenericCreateAction).Ref.(*v1.Service)
		}
	}
...
func TestKeycloakReconciler_Test_Setting_Resources(t *testing.T) {
	...
	assert.IsType(t, model.PostgresqlDeployment(cr), desiredState[3].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr), nil), desiredState[8].(common.GenericCreateAction).Ref)
	keycloakContainer := desiredState[8].(common.GenericCreateAction).Ref.(*v13.StatefulSet).Spec.Template.Spec.Containers[0]
	...
	postgresContainer := desiredState[3].(common.GenericCreateAction).Ref.(*v13.Deployment).Spec.Template.Spec.Containers[0]
